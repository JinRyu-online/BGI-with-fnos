...
- **`LogHarvester`** in `log_harvester.py` — background thread tails BetterGI log (supports glob patterns for daily logs), bridges to asyncio via `run_coroutine_threadsafe`, feeds the `/ws/logs/{job_id}` WebSocket for real-time log streaming. 启动后若目标文件不存在，每 0.5s 轮询等待，最多等 **5 分钟**（覆盖冷启动 + 慢速加密盘 + 长加载链），超时则放弃并打 `log file never appeared`。glob 模式选文件时优先选文件名含**当天日期**（YYYYMMDD）的文件，而非 mtime 最新，避免跨日误 tail 昨日残留。**完成判定 C 计数**:`set_keyword(k)` 启用计数模式,收割线程里持久累加 keyword 命中次数(每个文件行只过一遍,不依赖 deque 缓冲),`check_keyword` 在 `命中数 >= required_matches` 时返回 True。`required_matches` 默认=1(单组任务首次命中即触发);多组任务由 launcher 注入 `required_matches=len(groups)`,只有最后一组结束才触发,避免中间组误触发(BetterGI 每个组结束都打 `→ "任务结束"`)。未调 `set_keyword` 时回退到缓冲扫描模式(向后兼容)。
