...
Write-Host "[3b/5] 配置防火墙规则（放行 TCP 18765）..." -ForegroundColor Cyan
$FWNAME = "BGI-Trigger Listener"
netsh advfirewall firewall delete rule name="$FWNAME" | Out-Null
netsh advfirewall firewall add rule name="$FWNAME" dir=in action=allow protocol=TCP localport=18765
if ($LASTEXITCODE -ne 0) {
    Write-Host "[警告] 防火墙规则添加失败（可手动放行 TCP 18765 端口）。" -ForegroundColor Yellow
} else {
